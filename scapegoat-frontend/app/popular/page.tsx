...
async function getPopularPosts(page: number = 1): Promise<{ data: Post[]; pagination: PaginationData }> {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api';
  
  try {
    // Fetch lebih banyak posts untuk filter client-side
    ...
    // Filter hanya post yang popular
    const popularPosts = result.data ? result.data.filter((post: Post) => 
      post.is_popular === true || post.is_popular === 1 || post.is_popular === '1'
    ) : [];
    
    // Manual pagination karena filter di client side
    const perPage = 9;
    const total = popularPosts.length;
    const lastPage = Math.ceil(total / perPage);
    const startIndex = (page - 1) * perPage;
    const endIndex = startIndex + perPage;
    const paginatedPosts = popularPosts.slice(startIndex, endIndex);
...
export default async function PopularPostsPage({
  ...
  const { data: posts, pagination } = await getPopularPosts(currentPage);
        ...
        <section className="container mx-auto px-4 py-8">
          {posts.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-400">Belum ada postingan popular.</p>
            </div>
          ) : (
            <>
